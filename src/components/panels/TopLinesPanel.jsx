...
const TopLinesPanel = ({ bestLines }) => (
  <div className="flex h-[160px] flex-col rounded-xl border border-slate-900 bg-[#16161d] p-3 shadow-lg">
    ...
    <div className="mt-2 flex-1 space-y-1 overflow-y-auto pr-1">
      {bestLines.map((line, idx) => (
        ...
        >
          <summary className="flex items-start gap-1 list-none cursor-pointer">
            <span
            ...
            <span className="flex-1 min-w-0 text-xs font-mono text-slate-400 truncate">{line.san}</span>
          </summary>
          <div className="mt-1 text-xs font-mono text-slate-400 whitespace-pre-wrap break-words">{line.san}</div>
        </details>
      ))}
      {bestLines.length === 0 && (
